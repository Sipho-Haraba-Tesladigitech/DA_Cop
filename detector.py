...
from FeatureExtraction import get_values
from sklearn.preprocessing import StandardScaler
from pickle import load
import numpy as np

def pre_proc(website):
    '''
        * Gets the website base url and extracts features
        * Transform the feature values using standard scaler and returns the transformed values
    '''
    feat_values = get_values(website) #Get the features from the feature extractor
    feat_values = [0.0 if x == float('inf') else x for x in feat_values]
    # print(feat_values)
...
def detection(website):
    rf_model = load(open('rf_model.pkl', 'rb'))  #Load the model from the pickel file
